...
if($row>0)
{
	...
	$msg=mysqli_query($link,"insert into users(fname,lname,email,password,contactno) values('$fname','$lname','$email','$enc_password','$contact')");
...
$ret= mysqli_query($link,"SELECT * FROM users WHERE email='$useremail' and password='$dec_password'");
...
$row1=mysqli_query($link,"select email,password from users where email='$femail'");
